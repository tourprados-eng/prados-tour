...
create table public.trips(
 ...
 description text, itinerary text, included text, not_included text, rules text, cancellation_policy text, status public.trip_status not null default 'RASCUNHO',
 created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
...
create table public.bookings(
 ...
 quantity int not null check(quantity>0), boarding_point_id uuid references public.boarding_points(id), boarding_point text, total_amount numeric(12,2) not null check(total_amount>=0), base_amount numeric(12,2) not null check(base_amount>=0), discount_amount numeric(12,2) default 0,
...
create table public.booking_passengers(id uuid primary key default gen_random_uuid(),booking_id uuid references public.bookings(id) on delete cascade,name text not null,cpf text,birth_date date,phone text,seat_id uuid references public.seats(id),boarding_point_id uuid references public.boarding_points(id),travel_together boolean default true,group_id uuid,group_name text,observations text);
...
begin insert into public.profiles(id,full_name,cpf,birth_date,email,phone,whatsapp) values(new.id,coalesce(new.raw_user_meta_data->>'nome','Cliente'),coalesce(new.raw_user_meta_data->>'cpf',concat('PENDING-',new.id::text)),nullif(new.raw_user_meta_data->>'nascimento','')::date,new.email,new.raw_user_meta_data->>'telefone',new.raw_user_meta_data->>'whatsapp'); return new; end; $$;
...
create policy trips_public on trips for select using(status='PUBLICADA' or is_staff()); create policy trips_admin_write on trips for all using(is_admin()) with check(is_admin());
create policy images_public on trip_images for select using(exists(select 1 from trips t where t.id=trip_id and (t.status='PUBLICADA' or is_staff()))); create policy images_admin on trip_images for all using(is_admin()) with check(is_admin());
...
create policy tbp_public on trip_boarding_points for select using(exists(select 1 from trips t where t.id=trip_id and (t.status='PUBLICADA' or is_staff()))); create policy tbp_admin on trip_boarding_points for all using(is_admin()) with check(is_admin());
...
create policy coupons_public on coupons for select using(active=true); create policy coupons_staff on coupons for all using(is_staff()) with check(is_staff()); create policy coupon_usage_staff on coupon_usages for all using(is_staff() or user_id=auth.uid()) with check(is_staff() or user_id=auth.uid());
...
create or replace view public.admin_dashboard as select
...
begin
 ...
 if not found or v_trip.status<>'PUBLICADA' then raise exception 'TRIP_UNAVAILABLE'; end if;
